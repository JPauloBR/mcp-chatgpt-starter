...
export default function App() {
  ...
  const places = productsData?.locations || [];
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-50">
      ...
      {displayMode === "fullscreen" && <Header />}
      
      <div
        style={{
          maxHeight: displayMode === "fullscreen" ? undefined : maxHeight,
          height: displayMode === "fullscreen" ? "calc(100vh - 64px)" : 480,
        }}
        className={
          "relative antialiased w-full overflow-hidden " +
          (displayMode === "fullscreen"
            ? "rounded-none border-0 flex-1"
            : "border border-black/10 dark:border-white/10 rounded-2xl sm:rounded-3xl min-h-[480px]")
        }
      >
        <Outlet />
        {displayMode !== "fullscreen" && (
        ...
        {displayMode === "fullscreen" && (
          <div className="hidden antialiased md:flex absolute inset-x-0 bottom-2 z-30 justify-center pointer-events-none">
            <div className="flex gap-3 pointer-events-auto">
              {["Cell phones", "Broadband & Fiber", "Wireless plans", "5G coverage"].map((label) => (
                <button
                  key={label}
      ...
      {/* Footer - only show in fullscreen mode */}
...
createRoot(document.getElementById("att-store-finder-root")).render(
  <BrowserRouter>
    <RouterRoot />
  </BrowserRouter>
);
